	...
	public boolean performOk() {
		...
		if(!current.equals(previously)){
			...
			if(!previously.equals(PreferenceOptions.DefaultFeatureEditorDialog)){
				eval.executeFinish(registry, previously, CAEXFileNewProjectNature.ID);
			}
			...
			if(!current.equals(PreferenceOptions.DefaultFeatureEditorDialog)){
				...
				eval.executeInit(registry, current, CAEXFileNewProjectNature.ID, listMetamodelURIs);
		...
		if(!currentEngine.equals(previouslyEngine)) {
			...
			if(!currentEngine.equals(PreferenceOptions.DEFAULT_ENGINE)){
				...
				getDependencies ("http://www.dke.de/CAEX/300", listMetamodelURIs);
				listMetamodelURIs.add("http://www.dke.de/CAEX/300");
				eval.executeInit(registry, currentEngine, CAEXFileNewProjectNature.ID, listMetamodelURIs);				
			}			
		}
