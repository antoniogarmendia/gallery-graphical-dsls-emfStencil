...
public class SystemUnitClassLibCreateFolderWizard extends Wizard implements INewWizard {
	private SystemUnitClassLibCreateFolderWizardPage page;
	private ISelection selection;

	/**
	 * Constructor for SystemUnitClassLibCreateFolderWizard.
	 */
	public SystemUnitClassLibCreateFolderWizard() {
		super();
	...
	public void addPages() {
		page = new SystemUnitClassLibCreateFolderWizardPage(selection);
		addPage(page);
	}
	...
	private void doFinish(
		...
		URI fileURI = URI.createPlatformResourceURI(folder.getFullPath().toString().concat("/").concat(folder.getName()+".4"), true);
		//Create Root
		SystemUnitClassLib root_obj = CAEXFactoryImpl.eINSTANCE.createSystemUnitClassLib();
		//Set the Attributes
		...
		((EList<EObject>)file_xmi.Get_Root().eGet(file_xmi.Get_Root().eClass().getEStructuralFeature("systemUnitClassLib"), true)).add(folder_xmi.Get_Root());
		file_xmi.Save_XMI();				
