...
public class RoleClassLibCreateFolderWizard extends Wizard implements INewWizard {
	private RoleClassLibCreateFolderWizardPage page;
	private ISelection selection;

	/**
	 * Constructor for RoleClassLibCreateFolderWizard.
	 */
	public RoleClassLibCreateFolderWizard() {
		super();
	...
	public void addPages() {
		page = new RoleClassLibCreateFolderWizardPage(selection);
		addPage(page);
	}
	...
	private void doFinish(
		...
		URI fileURI = URI.createPlatformResourceURI(folder.getFullPath().toString().concat("/").concat(folder.getName()+".3"), true);
		//Create Root
		RoleClassLib root_obj = CAEXFactoryImpl.eINSTANCE.createRoleClassLib();
		//Set the Attributes
		...
		((EList<EObject>)file_xmi.Get_Root().eGet(file_xmi.Get_Root().eClass().getEStructuralFeature("roleClassLib"), true)).add(folder_xmi.Get_Root());
		file_xmi.Save_XMI();				
